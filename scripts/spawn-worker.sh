...
# ---- Persist the task description as DATA, never as shell ----
# The task description is untrusted input. It must never be interpolated into a
# generated script or an unquoted heredoc: markdown backticks and $(...) would be
# executed as commands at worker launch. printf %s writes it verbatim, and every
# consumer reads it from this file. See
# .orchestrator/decisions/D001-spawn-injection-and-false-done.md
TASK_FILE="$W_DIR/task-desc.txt"
printf '%s\n' "$TASK_DESC" > "$TASK_FILE" || die "Failed to write task file $TASK_FILE" 1
...
# The task text is written from the file, NOT interpolated into the heredoc
# above: an unquoted heredoc executes backticks and $(...) in the task. Fenced
# so markdown in the brief cannot restructure this document. (D001)
{
  printf '```text\n'
  cat "$TASK_FILE"
  printf '```\n\n'
...
# TASK= is deliberately a pointer, not the text. Inlining it both executed
# backticks (unquoted heredoc) and broke this key=value file whenever the task
# spanned multiple lines - which every real brief does. (D001)
...
# Values reach launch.sh through this env file, %q-quoted so no value can break
# out of its assignment. launch.sh itself is written with a QUOTED heredoc
# delimiter, so nothing in it expands at generation time. Together these mean the
# task description is never shell code. (D001)
LAUNCH_ENV="$W_DIR/launch.env"
{
  printf 'WORKER_ID=%q\n'      "$WORKER_ID"
  printf 'WT_PATH=%q\n'        "$WT_PATH"
  printf 'ADAPTER=%q\n'        "$ADAPTER"
  printf 'ADAPTER_SCRIPT=%q\n' "$ADAPTER_SCRIPT"
  printf 'TASK_TYPE=%q\n'      "$TASK_TYPE"
  printf 'TASK_FILE=%q\n'      "$TASK_FILE"
  printf 'OUTPUT_MD=%q\n'      "$OUTPUT_MD"
  printf 'M_PATH=%q\n'         "$M_PATH"
...
#!/usr/bin/env bash
# Generated by spawn-worker.sh. Quoted heredoc: nothing here was expanded at
# generation time. All values come from the sibling launch.env; the task text is
# read from a file and only ever handled as data.
set -e
...
echo "[worker $WORKER_ID] Task type: $TASK_TYPE (brief: $TASK_FILE)"
echo ""
...
# Only append an auto-marker if the worker did not already end the file with one.
...
if command -v jq >/dev/null 2>&1; then
  TMP=$(mktemp)
  FINAL_STATUS="$(detect_output_status "$OUTPUT_MD")"
  [ "$FINAL_STATUS" = "RUNNING" ] && FINAL_STATUS="FAILED"
  jq --arg ts "$(date -u +%Y-%m-%dT%H:%M:%SZ)" --arg st "$FINAL_STATUS" --argjson ec "$EXIT_CODE" \
     '.status=$st | .updated_at=$ts | .exit_code=$ec' "$M_PATH" > "$TMP" 2>/dev/null \
     && mv "$TMP" "$M_PATH" || rm -f "$TMP"
fi
